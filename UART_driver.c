...
void UART_init(unsigned long clock_speed){
	...
	uint8_t ubrr = 31;//(clock_speed / (BAUD * 16)) - 1;	//USART Baud Rate Register. Alltid et helt tall? 
	...
	set_bit(UCSR0C,UCSZ01);		// UCSZ1: 1		-	 UCSZ0 and UCSZ1 defines 8 bits (11 binary) for each character
